...
(*first, we evaluate e2. Then, we add e2 bound to p to the old
  environment whent he function was made. Then, we evaluate the function
  in the new environment, which looks for already-declared variables
  (including our argument.)*)
(*TODO add specs*)
...
(* match b with | Add -> (eval_expr env e1) + (eval_expr env e2) | Sub
   -> (eval_expr env e1) - (eval_expr env e2) | Mul -> (eval_expr env
   e1) * (eval_expr env e2) | Div -> (eval_expr env e1) / (eval_expr env
   e2) | Mod -> (eval_expr env e1) % (eval_expr env e2) | And ->
   (eval_expr env e1) && (eval_expr env e2) | Or -> (eval_expr env e1)
   || (eval_expr env e2) | Lt -> (eval_expr env e1) < (eval_expr env e2)
   | Le -> (eval_expr env e1) <= (eval_expr env e2) | Gt -> (eval_expr
   env e1) > (eval_expr env e2) | Ge -> (eval_expr env e1) >= (eval_expr
   env e2) | Eq -> (eval_expr env e1) = (eval_expr env e2) | Ne ->
   (eval_expr env e1) <> (eval_expr env e2) | Cat -> (eval_expr env e1)
   ^ (eval_expr env e2) | Pipe -> (eval_expr env e1) |> (eval_expr env
   e2) | Cons -> (eval_expr env e1) :: (eval_expr env e2) | Assign ->
   ref (eval_expr env e1) := (eval_expr env e2) | Bind -> let x =
   (eval_expr env e1) in (eval_expr env e2) *)

(*extract the pattern and expression - if the pattern and expression are
  both primitives, then return the inputted environment if the pattern
  is a PVariable, then add to the env*)
